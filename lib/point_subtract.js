...
var circle_loc = null;
if (flag_debug) {
    // shooting for 4 trials within 5 minutes
    /*
    var time_limit = 30 * 60 * 1000; // ms
    var main_seq = [1, 1, 1]; // [Infinity].concat(jsPsych.randomization.shuffle([3, 3, 6, 9, 9]));
    var break_after_trial = [2];
    */
    // shooting for 24 trials within 20 minutes
    var time_limit = 18 * 60 * 1000; // ms
    var main_seq = [Infinity].concat(jsPsych.randomization.shuffle([1, 2, 3, 4, 5, 6, 7, Infinity, Infinity]));
    main_seq = main_seq.concat(jsPsych.randomization.shuffle([1, 2, 3, 4, 5, 6, 7, Infinity, Infinity, Infinity]));
    main_seq = main_seq.concat(jsPsych.randomization.shuffle([1, 2, 3, 4, 5, 6, 7, Infinity, Infinity, Infinity]));
    var break_after_trial = [3, 7, 11, 15, 19, 23, 27];
...
function generate_practice_block(takeBackAmount = 0, smileyGoneBeforeHarvest = true, audioTest = true) {
    ...
    var prac_seq = [7, 7, 7, 7, 7, 7]; // [3, 7, 3, 7, 3, 7];
    var audio_testseq = jsPsych.randomization.shuffle([...audio_seed]);
...
function generate_main_block(takeBackAmount = 0, smileyGoneBeforeHarvest = true, audioTest = true) {
    ...
    var block_start_page = {
        type: 'instructions',
        pages: [
            '<div class = centerbox><p class = block-text>Take a deep breath, and click Next to begin the main task!</p></div>'
        ],
        allow_keys: false,
        show_clickable_nav: true,
