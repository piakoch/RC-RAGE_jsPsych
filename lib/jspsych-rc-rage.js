...
jsPsych.plugins['rc-rage'] = (function () {
    ...
    plugin.trial = function (display_element, trial) {
        ...
        const { scoreboard_gatherer_display_text, scoreboard_robber_display_text, takeback_amount, steal_amount } = trial;
        ...
        */
        ...
        jQuery("#jspsych-current-point").text(String(points_trial + cumulative_points_player));
        jQuery("#jspsych-current-point-robber").text(String(points_trial_robber + cumulative_points_robber));
        ...
        // utility functions to update the score display and update the score state variables
        ...
        const takeBackPoints = () => {
            points_trial += takeback_amount;
            points_trial_robber -= takeback_amount;
            updateScoreDisplay();
        };
        ...
        document.getElementById("green-circle").onclick = function (evt) {
            ...
            if (click_history.length == trial.steal_timing) {
                // steal the points
                points_trial -= steal_amount;
                points_trial_robber += steal_amount;
                updateScoreDisplay();
                // deliver the bad news
            ...
            if (circle_click >= trial.harvest_click) {
                ...
                jQuery("#jspsych-current-point").text((points_trial + trial.cumulative_points_player).toString());
                circle_click = 0;
